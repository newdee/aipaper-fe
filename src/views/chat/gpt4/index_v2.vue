...
export default {
  ...
  methods: {
    getOwnAvar() {
      //获取用户的头像
      this.own_avatar =
        "https://www.wandou.online/" + this.$store.getters.avatar;
      console.log(" this.$store.getters.avatar", this.own_avatar);

      this.own_avatar =
        this.own_avatar == "" || this.own_avatar == null
    ...
    send(refreshValue) {
      ...
      eventSource.onmessage = (event) => {
        ...
        this.infoList[this.infoList.length - 1].mdAnswer =
          marked.parse(resText);

        if (
          this.infoList[this.infoList.length - 1].mdAnswer.includes("<input")
        ) {
          this.infoList[this.infoList.length - 1].mdAnswer = this.infoList[
            this.infoList.length - 1
          ].mdAnswer.replaceAll("<input", "&lt;input");
        }
